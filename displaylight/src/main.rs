use displaylight::{DisplayLight, Config};
use lights;
...
fn main() -> Result<(), Box<dyn Error>> {
    let mut app = App::new("DisplayLight")
        ...
        .subcommand(
            SubCommand::with_name("list_ports")
                .about("List serial ports / com ports and quit.")
        );
    ...
    {
        let path = config_path.as_ref().unwrap().to_str().expect("Path is always valid.");
        println!("Config: {}", path);
        ...
        match File::open(path) {
            Ok(mut file) => {
                ...
                file.read_to_string(&mut content).expect("Should be able to read the file.");
