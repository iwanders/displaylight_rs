mod messages;
use messages::{Message, MsgType, ColorData};
...
impl Lights
{
    ...
    pub fn set_leds(&mut self, pixels: &[RGB]) -> Result<(), Box<dyn Error>>
    {
        ...
        let chunk_count = (pixels.len() as f32 / ColorData::LEDS_PER_MESSAGE as f32).ceil() as usize;
        for (i, chunk) in pixels.chunks(ColorData::LEDS_PER_MESSAGE).enumerate()
        {
            ...
            msg.payload.color.settings = if is_final {ColorData::SETTINGS_SHOW_AFTER} else {0};
            let mut colors: [RGB; ColorData::LEDS_PER_MESSAGE] = Default::default();
